...
def stock_return():
    ...
    return data
def trade_bot(starting_cash,step,step2):
    trade = 0
    B_S_FLAG = 'B'
    buy_level=0
    sell_level=0
    ...
    for i in range(len(adj_close)):
        ...
        if B_S_FLAG == "B": ##checks if we are currently looking to buy
            while x<=step:
                if adj_close.iat[i-y,0]>=adj_close.iat[i-x,0]: count +=1 
                y+=1
                x+=1
            if count == step:
                            B_S_FLAG = "S"
                            trade +=1
                            buy_level = adj_close.iat[i,0]
        ...
        df1={'Date':adj_close.index[i],'Cash':cash,'Price':adj_close.iat[i,0],'Buy/Sell':B_S_FLAG,'Buy Level':buy_level,'Return':daily_return,'Total Return':cash_return}
        data = data.append(df1,ignore_index = True)  
    return data
def graphing(bot):
